...
<style>
    ...
    .stButton>button {border-radius: 6px; background-color: #007bff; color: white; padding: 10px 28px; border: none; box-shadow: 0 2px 5px rgba(0,0,0,0.15); transition: background-color 0.2s ease, transform 0.1s ease; font-weight: 500; font-size: 1.05em;}
...
def book_query_page():
    ...
    with st.form("search_form"):
        ...
        with cols[0]: book_name_search = st.text_input("书名", value=st.session_state.query_conditions.get('bookname',''), placeholder="输入书名关键词...")
        with cols[1]: author_search = st.text_input("作者", value=st.session_state.query_conditions.get('author',''), placeholder="输入作者名...")
        with cols[2]: publisher_search = st.text_input("出版社", value=st.session_state.query_conditions.get('publishdepartment',''), placeholder="输入出版社名...")
        ...
        if year_from_session:
            ...
            except ValueError: default_year_val = None
        with cols[3]: publish_year_search = st.number_input("出版年份", min_value=1000, max_value=datetime.now().year + 5, step=1, value=default_year_val, format="%d", placeholder="YYYY")
        st.markdown("<br>", unsafe_allow_html=True)
        search_button_cols = st.columns([1,0.3,1])
        with search_button_cols[1]: search_submitted = st.form_submit_button("开始查询", use_container_width=True)
    ...
    if active_conditions:
        ...
        if not results_df.empty:
            ...
            # --- MODIFIED HEIGHT CALCULATION FOR FIXED 15 ROWS VISUAL HEIGHT ---
            # Estimate pixel height for header and each row. Adjust these based on your visual output.
            header_height_px = 60  # Approximate height of the table header row
            row_height_px = 37     # Approximate height of a single data row
            buffer_px = 5          # Small buffer to prevent unexpected scrollbars
            
            # Calculate the height needed for a full page of ITEMS_PER_PAGE rows
            fixed_height_for_15_rows = header_height_px + (ITEMS_PER_PAGE * row_height_px) + buffer_px
            
            # Apply this fixed height whenever there's data to display
            st.dataframe(display_df[display_columns_present], use_container_width=True, height=int(fixed_height_for_15_rows))
            # --- END OF MODIFIED HEIGHT CALCULATION ---
