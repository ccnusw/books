...
def book_query_page():
    ...
    with st.form("search_form"):
        ...
        # --- MODIFICATION: Adjust columns for button centering ---
        # Create 5 columns: [spacer_left, search_button, small_gap, clear_button, spacer_right]
        # The relative ratios will determine how centered they appear.
        # e.g., [1.5, 1, 0.1, 1, 1.5] -> spacers are 1.5x width of buttons
        # e.g., [2, 1, 0.1, 1, 2] -> spacers are 2x width of buttons (more centered)
        # You might need to adjust these ratios based on the overall page width and desired centering.
        spacer_ratio = 1.5 # Adjust this to control centering
        button_ratio = 1   # Keep button width reasonable
        gap_ratio = 0.1    # Small gap between buttons

        cols_buttons = st.columns([spacer_ratio, button_ratio, gap_ratio, button_ratio, spacer_ratio]) 
        with cols_buttons[1]: # Search button in the second segment
            search_submitted = st.form_submit_button("开始查询", use_container_width=True)
        with cols_buttons[3]: # Clear button in the fourth segment
            clear_submitted = st.form_submit_button("清空所有", use_container_width=True, type="secondary")
        # cols_buttons[0], cols_buttons[2], cols_buttons[4] are spacers
        # --- END OF MODIFICATION ---
